...
The agent's system prompt SHALL establish it as a headless sub-agent that never initiates user interaction. Like the apply agent, it SHALL map any skill instruction to "ask the user" into returning structured status to the dispatcher.
...
#### Scenario: Adaptation guidance present
- **WHEN** the agent system prompt is read
- **THEN** it maps "ask the user" and "wait for guidance" instructions to dispatcher-return equivalents
